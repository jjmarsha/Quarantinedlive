...
import {ParseDateString, ParseTimeString, ParseDateTimeToUTC} from "../../lists/daterefs";
...
const Entry = ({ entry, className, onClick, isMobile, history }) => {
  ...
  const ParseDate = () => {
    ...
    if(entry.time) return `${ParseDateString(date.toString())} ${ParseTimeString(date.toString().split(" ")[4])}`;
  }
  ...
  if (isMobile) {
    return (
      <div className={"entry-mobile " + className} onClick={redirectEntryView}>
        <Row className="h-100">
          <Col xs="1" className="m-auto text-right">
          </Col>
          <Col
            xs="10"
  ...
  return (
    <div className={"entry border-left-success " + className} onClick={redirectEntryView}>
      <Row className={"h-100 entry" + className}>
        ...
        <Col md="4" className="mt-auto mb-auto pl-0 entry-text-small text-left">
          Topics(s):&nbsp;
          {entry.topics ? entry.topics.map((topic, key) => {
            return <>{(key === 0 ? "" : ", ") + topic}</>;
          }): null}
        </Col>
      </Row>
